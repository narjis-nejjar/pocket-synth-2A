...
volatile uint8_t effect_mode = 255;
...

{
 ...
 /* 2. Filtre IIR seulement si effet 0 */
 if (effect_mode == 0)
...
/* USER CODE END WHILE */
 ...
 key = Keypad_Scan();
 if (key == 'A') { if (effect_mode == 0) effect_mode = 255; else { effect_mode = 0; w[0]=w[1]=w[2]=0.0f; } }
 if (key == 'B') { if (effect_mode == 1) effect_mode = 255; else effect_mode = 1; }
 if (key == 'C') { if (effect_mode == 2) effect_mode = 255; else effect_mode = 2; }
 ...
 if (sequencer_running && effect_mode == 255)
     {
         effect_mode = 0;  // active IIR par défaut si aucun effet
         w[0]=w[1]=w[2]=0.0f;
     }
 ...
 */
 ...
 RCC_ClkInitStruct.ClockType = RCC_CLOCKTYPE_HCLK|RCC_CLOCKTYPE_SYSCL
