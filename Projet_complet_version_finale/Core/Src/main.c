...

{
 ...
 if (effect_mode == 2)
     sample = 2.0f * phase - 1.0f;  // dents de scie
 else
     sample = (phase < 0.5f) ? 1.0f : -1.0f;  // carré
 ...
 if (effect_mode == 255)
     HAL_DAC_SetValue(&hdac1, DAC_CHANNEL_1, DAC_ALIGN_12B_R, 2048); // silence
 else
 {
...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  if (key == 'B')
	  {
	      if (effect_mode == 1) effect_mode = 255;
	      else effect_mode = 1;
	  }
	   if (key == 'C')
	  {
	      if (effect_mode == 2) effect_mode = 255;
	      else effect_mode = 2;
	  }
	  if (key == 'D')
	  {
	      if (effect_mode == 0) effect_mode = 255;
	      else { effect_mode = 0; w[0]=w[1]=w[2]=0.0f; }
	  }
	  if (key != 0 && key != 'A' && key != 'B' && key != 'C' && key != 'D' && !sequencer_running)
	  {
	      for (int r = 0; r < 4; r++)
	          for (int c = 0; c < 4; c++)
	              if (keypad[r][c] == key)
